...
from ncxlib.activations import LeakyReLU, Sigmoid
from ncxlib.losses import BinaryCrossEntropy
from ncxlib.models.neuralnetwork.layers import Layer
...
class OutputLayer(Layer):
    def __init__(
        ...
        if loss_fn == BinaryCrossEntropy and isinstance(
            self.layer.activation, LeakyReLU
        ):
    ...
    def back_propagation(self, y_true: np.ndarray, learning_rate: float) -> None:
        ...
        self.layer.W, self.layer.b = self.layer.optimizer.apply(
            self.layer.W, dl_dw, self.layer.b, dl_db
        )
